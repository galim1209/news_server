...
document.addEventListener("DOMContentLoaded", ()=> {
    document.querySelectorAll(".btn-edit").forEach( btn => {
        btn.addEventListener("click",(e)=>{
            ...
            // 편집 버튼을 완료 버튼으로 변환 : 펴지 버튼을 감추고 완료버튼을 보이게 하기
            btn.classList.add("d-none");

            // update form 을 보이게 하기 
            ...
            updateForm.addEventListener("submit", (evt)=>{
                ...
                if(newName === originName & newMemo === originMemo){
                    //값이 변경되지 앟았다면 모든 엘리먼트(컨트롤, 컴포넌트)들을 원래대로 복귀시켜놓고 빠져나감
                    evt.preventDefault();   // 새로고침 방지 
                    // ui 복구
                    nameInput.classList.add("d-none");  // nameInput 박스 숨기기 
                    memoInput.classList.add("d-none");  // memoInput 박스 숨기기 
                    // span 엘리먼트를 보이게 하기 
                    row.querySelector("span").forEach(elem => elem.classList.remove("d-none"));
                    // form 감추기 
                    updateForm.classList.add("d-none");
                    btn.classList.remove("d-none"); // edit 버튼을 보이게 나타내기
                    return;
                }

                // 변경된 값을 hidden input에반영
